            ...
            if (urlObj.pathname.match(/\.json[p]*$/)) {
                ...
                if (type == 'js') {
                    ...
                    if (filename == 'version.js') {
                        ...
                        res.end('window._version = "' + (new Date().getTime().toString(16)) + '";');
                        req.connection.destroy();
                        debug.pushLog('Server', 'Response for "Version JS" is ready.', 'info');
                    } else {
                    ...
                    combineCSS(filename, projects[project], {
                        compress : compress,
                        version : new Date().getTime().toString(16)
                    }, function(data){
                    ...
                    if(fs.existsSync(filePath)) {
                        if (filePath.match(/\.html$/)){
                            var info = fs.readFileSync(filePath, 'utf-8').replace(/^\xef\xbb\xbf/,''), version = new Date().getTime().toString(16);
